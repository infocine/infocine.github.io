...
//Warning la connexion ne se fait pas 
  ...
  if(!empty($_POST)){
    ...
    if(isset($_POST['inscription'])){
      ...
      if(empty($nom)){
        $valid = false;
        $erreur_nom = "Veuillez renseigner un nom !";
      }

      if(empty($mail)){
        $valid = false;
        $erreur_mail = "Veuillez renseigner un mail !";
      }
      ...
      if($valid){
        ...
        $req = $BDD->prepare("INSERT INTO utilisateur (nom, mail, password)  VALUES (?,?,?)");

        $req->execute(array($nom, $mail, $password));
